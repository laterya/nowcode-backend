...
public abstract class BaseService implements ApiService {
    ...
    public void checkConfig(NowApiClient nowApiClient) throws ApiException {
        if (nowApiClient == null && this.getNowApiClient() == null) {
    ...
    private <O, T extends ResultResponse> HttpRequest getHttpRequestByRequestMethod(BaseRequest<O, T> request) throws ApiException {
        ...
        return httpRequest.addHeaders(getHeaders(JSONUtil.toJsonStr(request), nowApiClient)).body(JSONUtil.toJsonStr(request.getRequestParams()));
    }
     ...
     */
    private Map<String, String> getHeaders(String body, NowApiClient nowApiClient) {
        Map<String, String> hashMap = new HashMap<>(4);
        hashMap.put("accessKey", nowApiClient.getAccessKey());
